...
export default function AudioPlayer() {
  ...
  // Only render if we have a user and either audio data or something is playing
  if (!user || (!audioData && !isPlaying)) {
    return null;
  }

  if (error) {
    return (
      <div className="w-full h-24 bg-red-900/20 border-t border-red-800 fixed bottom-0 left-0 right-0 z-50">
        <div className="h-full mx-auto px-4 flex items-center justify-between">
          <p className="text-red-400">Error playing audio: {error}</p>
          <Button 
            variant="destructive" 
  ...
  return (
    <div className="w-full h-24 bg-black/95 backdrop-blur-md border-t border-gray-800 fixed bottom-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out shadow-2xl">
        ...
        <div className="flex flex-col items-center gap-2 flex-1 max-w-[600px]">
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              disabled={!audioData}
              className={cn(
                "h-12 w-12 rounded-full border-none text-white transition-all duration-200",
                audioData
                  ? "bg-[#4CAF50] hover:bg-[#45a049] hover:scale-105"
            ...
            >
              <SelectTrigger
                className={cn(
                  "w-[80px] bg-transparent text-white",
