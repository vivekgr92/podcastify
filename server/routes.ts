    ...
    try {
      ...
      try {
          ...
          return res.status(403).json({
            ...
            limits: {
              ...
              tokens: {
                ...
                wouldExceed: currentTokens + estimatedTotalTokens > TOKEN_LIMIT,
              },
            },
            pricing: {
        ...
        await logger.info([
          ...
          `This conversion will use: ${estimatedTotalTokens} tokens`,
        ]);
        ...
        try {
          ...
          await logger.info(
            `Audio generation successful: ${usage.inputTokens} input tokens, ${usage.outputTokens} output tokens, ${duration}s duration`,
          );
          ...
          const result = await db.transaction(async (tx) => {
            ...
            const [updatedUsage] = await tx
              ...
              .onConflictDoUpdate({
                ...
                set: {
                  articlesConverted: sql`COALESCE("user_usage"."articlesConverted", 0) + 1`,
                  tokensUsed: sql`COALESCE("user_usage"."tokensUsed", 0) + ${estimatedTotalTokens}`,
                  lastConversion: new Date(),
                },
              })
              .returning();
          ...
          await logger.error(
            `Error generating audio: ${error instanceof Error ? error.message : String(error)}`,
          );
          throw error;
        }
      } catch (error) {
