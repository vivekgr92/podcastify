...
const pdfParse = async (buffer: Buffer) => {
  const parse = (await import('pdf-parse')).default;
  return parse(buffer);
};
...
try {
  ...
  logger.error(`Failed to initialize Stripe: ${error instanceof Error ? error.message : String(error)}`);
}

// Constants for usage limits and pricing
const ARTICLE_LIMIT = process.env.ARTICLE_LIMIT ? parseInt(process.env.ARTICLE_LIMIT) : 3;
const PODIFY_TOKEN_LIMIT = process.env.PODIFY_TOKEN_LIMIT ? parseInt(process.env.PODIFY_TOKEN_LIMIT) : 10000;
const PODIFY_TOKEN_RATE = process.env.PODIFY_TOKEN_RATE ? parseFloat(process.env.PODIFY_TOKEN_RATE) : 0.005;
const PODIFY_MARGIN = process.env.PODIFY_MARGIN ? parseFloat(process.env.PODIFY_MARGIN) : 0.6;
...
export function registerRoutes(app: Express) {
  try {
    ...
    // Only register TTS routes if service is available
    if (ttsService) {
            ...
            case "invoice.payment_succeeded":
              ...
              try {
                ...
                const [updatedUser] = await db
                  ...
                  .where(eq(users.id, parseInt(subscription?.metadata.userId as string)))
                  .returning();
            ...
            case "invoice.payment_failed":
              ...
              if (failedSubscription?.metadata.userId) {
                await db
                  ...
                  .where(
                    eq(users.id, parseInt(failedSubscription?.metadata.userId as string)),
                  );
            ...
            case "customer.subscription.updated":
              ...
              if (updatedSubscription.metadata.userId) {
                await db
                  ...
                  .where(
                    eq(users.id, parseInt(updatedSubscription.metadata.userId as string)),
                  );
            ...
            case "customer.subscription.deleted":
              ...
              if (deletedSubscription.metadata.userId) {
                await db
                  ...
                  .where(
                    eq(users.id, parseInt(deletedSubscription.metadata.userId as string)),
                  );
      ...
      logger.info('TTS service routes registered');
    }

    // Get user's podcasts (duplicate, remove one)

    logger.info('Route registration completed successfully');
    return app;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error(`Error in route registration: ${errorMessage}`);
    throw error;
  }
}
...
let ttsService: any;
try {
  ttsService = await import("./services/tts.js").then(m => m.ttsService);
} catch (error) {
  logger.warn("TTS service not available, some features will be disabled");
}

// Constants for usage limits and pricing (already defined above)